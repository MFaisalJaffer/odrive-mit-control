  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  sub_command_ = node->create_subscription<odrive_mit_example::msg::LegCmd>(
    ...
    [this](const odrive_mit_example::msg::LegCmd::SharedPtr msg) {
      // Store message for update loop
      // NOTE: In a real-time controller we should use a realtime buffer (e.g. realtime_tools::RealtimeBox)
      // For simplicity here, we just copy (non-RT safe potential).
      latest_cmd_ = *msg;
      has_new_cmd_ = true;
    });
